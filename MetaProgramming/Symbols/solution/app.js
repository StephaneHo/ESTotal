...
const uid2 = Symbol("uid2");

console.log(uid2); // this does not mean that this symbol now had an internal name,
//uid2, or anything like that, it is only an identification criterion
const uid2Other = Symbol("uid2");
console.log(uid2 === uid2Other); // false
...
person = {
  [uid2]: "id1", // if it is uid2 instead of [uid2], then you would have just added a regular key: uid2
  name: "Jean",
  age: 30,
};
...
// if you export the boject but you do not export the symbol, the consumer cannot interfere with the internal ID
// even if you try to recreate the symbol as a consumer, it won't work (uniquenss guarantee)
console.log(' person[Symbol("uid2")', person[Symbol("uid2")]);
